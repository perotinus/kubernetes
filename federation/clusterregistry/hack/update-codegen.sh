...
BASE_PKG=k8s.io/kubernetes/federation/clusterregistry
...
function generate_group() {
  ...
  ${listergen} --input-dirs ${BASE_PKG}/pkg/apis/registry --input-dirs ${BASE_PKG}/pkg/apis/${GROUP_NAME}/${VERSION} --output-package ${LISTERS_PKG} --output-base ${SERVER_BASE}
...
generate_group registry v1alpha1
